...
const AdminSettings: React.FC<AdminSettingsProps> = ({ 
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      {/* HEADER SECTION */}
      <div className="bg-green-800 p-8 rounded-[2.5rem] text-white shadow-xl shadow-green-100 mb-8">
        <h2 className="text-2xl font-black uppercase tracking-tight">Admin Pusat — Konfigurasi Sistem</h2>
        <p className="text-green-100 text-xs font-medium mt-2">Kelola durasi ujian, keamanan password, dan link bantuan eksternal di sini.</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
            <h2 className="text-sm font-black text-slate-800 mb-6 flex items-center gap-2 uppercase tracking-widest">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-800" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M2.166 4.9L10 .303 17.834 4.9a1 1 0 01.5 1.075l-1.334 8a1 1 0 01-.504.743l-6 3.5a1 1 0 01-.992 0l-6-3.5a1 1 0 01-.504-.743l-1.334-8a1 1 0 01.5-1.075zm2.131 2.22l.774 4.648L10 14.12l4.929-2.353.774-4.648L10 9.303 4.297 7.12z" clipRule="evenodd" /></svg>
              Keamanan & Akun Utama
            </h2>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
            <h2 className="text-sm font-black text-slate-800 mb-6 flex items-center gap-2 uppercase tracking-widest">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-800" viewBox="0 0 20 20" fill="currentColor"><path d="M7 3a1 1 0 000 2h6a1 1 0 100-2H7zM4 7a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1zM2 11a2 2 0 012-2h12a2 2 0 012 2v4a2 2 0 01-2 2H4a2 2 0 01-2-2v-4z" /></svg>
              Utilitas Data Lokal
            </h2>
            <div className="space-y-4">
              <div className="max-h-32 overflow-y-auto border border-green-100 rounded-2xl p-2 bg-green-50 custom-scrollbar">
                {availableTokens.map(token => (
                  <label key={token} className="flex items-center gap-3 p-2 hover:bg-white rounded-xl cursor-pointer">
                    <input type="checkbox" checked={selectedTokens.includes(token)} onChange={() => toggleToken(token)} className="w-4 h-4 accent-green-800" />
                    <span className="text-[10px] font-black text-slate-500 uppercase">{token}</span>
                  </label>
                ))}
              </div>
              <div className="grid grid-cols-1 gap-2">
                <button onClick={handleExportJSON} disabled={selectedTokens.length === 0} className="w-full bg-green-950 disabled:opacity-20 text-white font-black py-3 rounded-xl text-[10px] uppercase tracking-widest transition-all">Export JSON (Backup)</button>
                <input type="file" ref={fileInputRef} className="hidden" accept=".json" onChange={handleFileChange} />
                <button onClick={() => fileInputRef.current?.click()} className="w-full bg-green-50 text-green-800 font-bold py-3 rounded-xl border border-green-100 text-[10px] uppercase tracking-widest transition-all">Upload JSON (Restore)</button>
                <button onClick={() => setIsPasteModalOpen(true)} className="w-full bg-green-50 text-slate-600 font-bold py-3 rounded-xl border border-green-100 text-[10px] uppercase tracking-widest transition-all">Paste JSON (Manual)</button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
             ...
             <button 
              ...
              className="w-full bg-green-800 hover:bg-green-900 disabled:opacity-30 text-white font-black py-4 rounded-xl text-[10px] uppercase tracking-widest shadow-lg shadow-green-100 flex items-center justify-center gap-2 transition-all"
            >
      ...
      <div className="mt-10 p-4 sticky bottom-4 z-10">
        <button 
          ...
          className="w-full max-w-md mx-auto block bg-green-800 hover:bg-green-900 text-white font-black py-5 rounded-[2rem] shadow-2xl shadow-green-300 transition-all active:scale-95 uppercase tracking-[0.2em] text-xs"
        >
      ...
      {/* MODAL PASTE JSON */}
      {isPasteModalOpen && (
        <div className="fixed inset-0 bg-green-950/70 backdrop-blur-md z-[200] flex items-center justify-center p-4">
