    ...
    <div className="bg-white rounded-3xl shadow-sm border border-green-100 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-green-50/80 text-slate-400 text-[10px] uppercase font-black tracking-[0.2em] border-b">
            <tr>
              ...
              results.map(res => (
                <tr key={res.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-6 text-center">
                    <span className="bg-green-800 text-white px-4 py-1.5 rounded-full font-black text-sm shadow-lg shadow-green-100">{res.score.toFixed(1)}</span>
                  </td>
                  <td className="p-6 text-center">
                    {res.isCorrected ? 
                  ...
                  <td className="p-6 text-right">
                    <button 
                      ...
                      className="bg-green-950 hover:bg-black text-white text-[10px] font-black px-5 py-2.5 rounded-xl transition-all shadow-md active:scale-95 uppercase tracking-widest"
                    >
                      Buka Lembar Jawab
                    </button>
                  </td>
      ...
      {selectedResult && (
        <div className="fixed inset-0 bg-green-950/80 backdrop-blur-xl z-50 flex items-center justify-center p-6">
          <div className="bg-white rounded-[3.5rem] w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-[0_32px_64px_-12px_rgba(0,0,0,0.3)] border border-white/20">
            <div className="p-8 border-b bg-white flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setSelectedResult(null)} className="bg-green-50 hover:bg-green-100 p-3 rounded-full transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar bg-green-50/30">
              {questions.map((q, idx) => {
                const answer = selectedResult.answers[q.id];
                
                return (
                  <div key={q.id} className="bg-white p-8 rounded-[2.5rem] border border-green-100 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-green-50 -mr-16 -mt-16 rounded-full opacity-50"></div>
                    
                    <div className="relative flex flex-col md:flex-row gap-8">
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-6">
                          <span className="bg-green-100 text-slate-500 text-[10px] px-3 py-1 rounded-lg font-black uppercase tracking-widest">Butir Soal {idx+1}</span>
                        ...
                        <div className="p-5 bg-green-50/50 rounded-2xl border border-green-100 italic">
                           <p className="text-[10px] text-green-800 font-black uppercase mb-2 tracking-widest">Pembahasan:</p>
                           <p className="text-sm text-green-900 leading-relaxed" style={{ whiteSpace: 'pre-wrap' }}>{q.explanation || "Tidak ada pembahasan tersedia."}</p>
                        </div>
                      </div>

                      <div className="md:w-48 flex flex-col justify-end bg-green-50 p-6 rounded-3xl border border-green-100">
                         ...
                         <div className="flex flex-col gap-2">
                            <input 
                               ...
                               max="100" 
                               min="0"
                               className="w-full p-4 bg-white border-2 border-green-100 rounded-2xl text-center text-2xl font-black focus:border-green-800 outline-none transition-all"
                               placeholder="0"
            ...
            <div className="p-8 border-t bg-white flex flex-col md:flex-row gap-4 justify-between items-center shrink-0">
               ...
               <div className="flex gap-4 w-full md:w-auto">
                 ...
                 <button 
                  ...
                  className="flex-1 md:px-12 py-4 bg-green-800 hover:bg-green-900 text-white font-black rounded-2xl shadow-2xl shadow-green-200 transition-all active:scale-95"
                >
                  SIMPAN KOREKSI
                </button>
               </div>
