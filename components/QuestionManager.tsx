...
const QuestionManager: React.FC<QuestionManagerProps> = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative">
      <div className="p-4 border-b border-slate-200 bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
            <button onClick={() => setActiveTab('active')} className={`px-4 py-2 rounded-lg text-[10px] font-black ${activeTab === 'active' ? 'bg-green-800 text-white' : 'text-slate-400'}`}>BANK SOAL</button>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/30 p-4 space-y-4 custom-scrollbar">
        {processedQuestions.map((q) => (
          <div key={q.id} className="bg-white p-4 border rounded-2xl group flex gap-4 items-start shadow-sm hover:shadow-md transition-all">
            <div className="w-10 h-10 flex items-center justify-center bg-green-50 text-green-800 rounded-xl font-black text-xs shrink-0">{q.order}</div>
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start mb-2">
                <span className="text-[8px] bg-green-800 text-white px-2 py-0.5 rounded font-black uppercase">TOKEN: {q.quizToken}</span>
                <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-all">
                   ...
                   <button onClick={() => handleEdit(q)} className="text-green-800 text-[9px] font-black uppercase hover:underline">Edit</button>
      ...
      {previewQuestion && (
        <div className="fixed inset-0 bg-green-950/80 backdrop-blur-md z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] border border-white/20">
             <div className="p-6 bg-green-950 text-white flex justify-between items-center shrink-0">
             ...
             <div className="p-6 border-t bg-slate-50 flex justify-center">
                <button onClick={() => setPreviewQuestion(null)} className="px-12 py-3.5 bg-green-950 hover:bg-black text-white font-black rounded-2xl text-[10px] uppercase shadow-xl transition-all active:scale-95">TUTUP PRATINJAU</button>
             </div>
          </div>
        </div>
      )}
      ...
      {zoomImage && (
        <div className="fixed inset-0 z-[3000] bg-green-950/90 backdrop-blur-md flex items-center justify-center p-12 animate-in fade-in duration-300" onClick={() => setZoomImage(null)}>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-green-950/60 backdrop-blur-md z-[1500] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] w-full max-w-7xl p-8 shadow-2xl overflow-y-auto max-h-[95vh] custom-scrollbar flex flex-col">
                ...
                <div className="space-y-6">
                   ...
                   <div className="space-y-1">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <button 
                          onClick={() => setFormData({...formData, text: applyAutoLatex(formData.text)})}
                          className="text-[9px] font-black bg-purple-100 text-purple-700 px-2 py-1 rounded-lg hover:bg-purple-200 transition-colors uppercase flex items-center gap-1"
                        >
                          ✨ Auto-LaTeX
                        </button>
                      </div>
                      <textarea value={formData.text} onChange={e => setFormData({...formData, text: e.target.value})} className="w-full p-4 border bg-slate-50 rounded-2xl h-32 font-mono text-sm outline-none focus:border-green-800 focus:bg-white transition-all mb-3" placeholder="Tulis soal di sini..." />
                      <div className="space-y-1">
                              ...
                              <div className="flex gap-3 items-start">
                                 <div className="pt-2">
                                    <input 
                                      ...
                                      className="w-4 h-4 cursor-pointer accent-green-800"
                                    />
                                 </div>
                                 <div className="flex-1 relative">
                                    ...
                                    }} className="w-full p-3 pr-10 bg-white border rounded-xl text-xs font-mono h-16 outline-none focus:border-green-800 transition-all" placeholder={`Teks Opsi ${String.fromCharCode(65+idx)}...`} />
                                    <button 
                                      onClick={() => {
                                        const next = [...formData.options];
                                        next[idx] = applyAutoLatex(next[idx]);
                                        setFormData({...formData, options: next});
                                      }}
                                      className="absolute right-2 top-2 p-1.5 bg-slate-100 hover:bg-purple-100 text-slate-400 hover:text-purple-600 rounded-lg transition-all"
                ...
                <div className="bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200 p-8 flex flex-col sticky top-20 h-fit max-h-[80vh] overflow-y-auto custom-scrollbar">
                   ...
                   <div className="bg-white p-8 rounded-3xl shadow-sm border border-green-100">
                      <div className="flex items-center gap-3 mb-6">
                         <div className="w-10 h-10 bg-green-800 text-white rounded-xl flex items-center justify-center font-black text-lg">{formData.order}</div>
                         ...
                         {formData.type === QuestionType.TRUE_FALSE || formData.type === QuestionType.MATCH ? (
                               ...
                               <tbody className="divide-y divide-green-50">
                                 {formData.options.map((opt, i) => (
                                   <tr key={i}>
                                     ...
                                     <td className="p-3 flex gap-1 justify-center">
                                       <div className={`px-2 py-1 rounded text-[8px] font-black ${formData.correctAnswer?.[i] === true ? 'bg-green-800 text-white' : 'bg-green-50 text-slate-300'}`}>{formData.tfLabels.true.toUpperCase()}</div>
                                       <div className={`px-2 py-1 rounded text-[8px] font-black ${formData.correctAnswer?.[i] === false ? 'bg-red-600 text-white' : 'bg-green-50 text-slate-300'}`}>{formData.tfLabels.false.toUpperCase()}</div>
                                     </td>
                                   </tr>
                                 ))}
                               </tbody>
                           ...
                           <div className="space-y-3">
                              {formData.options.map((opt, i) => {
                                 ...
                                 return (
                                   <div key={i} className={`flex flex-col p-4 border-2 rounded-2xl transition-all ${isCorrect ? 'border-green-800 bg-green-50' : 'border-green-50 bg-white'}`}>
                                      <div className="flex items-start">
                                         <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs mr-4 shrink-0 ${isCorrect ? 'bg-green-800 text-white' : 'bg-green-50 text-slate-400'}`}>{String.fromCharCode(65+i)}</div>
                                         <MathText text={opt || "..."} className={`flex-1 text-xs font-bold ${isCorrect ? 'text-green-900' : 'text-slate-700'}`} />
                                      </div>
             ...
             <div className="flex gap-4 mt-10 pt-6 border-t bg-white sticky bottom-0 z-20">
                ...
                <button onClick={handleSave} className="flex-[2] bg-green-800 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-green-900 transition-all uppercase text-xs">Simpan Perubahan</button>
             </div>
          </div>
        </div>
      )}
      ...
      {isPasteModalOpen && (
        <div className="fixed inset-0 bg-green-950/70 backdrop-blur-md z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-8 bg-green-950 text-white flex justify-between items-center shrink-0">
              <div>
            ...
            <div className="p-8 flex-1 overflow-y-auto">
              <textarea 
                className="w-full h-80 p-6 bg-green-50 border-2 border-green-100 rounded-3xl font-mono text-xs outline-none focus:border-green-800 transition-all"
            ...
            <div className="p-8 border-t bg-slate-50 flex gap-4">
              ...
              <button onClick={handlePasteImport} className="flex-[2] bg-green-800 text-white font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-[10px]">Import</button>
            </div>
          </div>
        </div>
      )}
