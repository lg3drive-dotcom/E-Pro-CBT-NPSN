...
const QuizInterface: React.FC<QuizInterfaceProps> = ({ questions, identity, timeLimitMinutes, subjectName, onFinish, onViolation }) => {
                   ...
                   <tr key={idx} className="hover:bg-slate-50 transition-colors">
                     ...
                     <td className="p-4">
                        <div className="flex bg-slate-100 p-1 rounded-xl gap-1">
                          ...
                          }} className={`flex-1 py-2 px-1 rounded-lg text-[9px] font-black transition-all ${val === true ? 'bg-green-800 text-white shadow-md' : 'text-slate-400'}`}>{labels.true.toUpperCase()}</button>
                          <button onClick={() => {
        ...
        <div className="space-y-4">
          {isMultiple && (
            <div className="flex items-center gap-2 mb-4 bg-green-50 p-3 rounded-xl border border-green-100">
              ...
              <p className="text-green-900 font-bold text-xs uppercase tracking-tight">Pilih lebih dari satu jawaban yang benar</p>
            </div>
          )}
          {q.options?.map((opt, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className={`w-full flex items-start p-4 text-left border-2 rounded-xl transition-all ${isSelected ? 'border-green-800 bg-green-50 shadow-inner' : 'border-slate-200 hover:bg-slate-50'}`}
              >
                <div 
                  onClick={() => {
                    if (isMultiple) {
                  ...
                  className={`w-10 h-10 flex items-center justify-center mr-4 shrink-0 transition-all cursor-pointer ${isMultiple ? 'rounded-lg' : 'rounded-full'} ${isSelected ? 'bg-green-800 text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}
                >
                  {isMultiple ? (
                    isSelected ? (
                ...
                <div className="flex-1 min-w-0">
                  <div onClick={() => {
                    ...
                    <MathText text={opt} className={`font-bold block ${isSelected ? 'text-green-900' : 'text-slate-700'}`} style={{ fontSize: `${fontSize - 2}px` }} />
                  </div>
                  {optImg && (
    ...
    return (
      <div className="min-h-screen bg-green-950 flex items-center justify-center p-6 text-center">
        <div className="bg-white rounded-[2rem] p-10 max-w-lg w-full shadow-2xl">
          ...
          <button onClick={requestFullscreen} className="w-full bg-green-800 text-white font-black py-4 rounded-2xl uppercase tracking-widest shadow-xl active:scale-95 transition-all">
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col lg:h-screen lg:overflow-hidden select-none">
       <header className="bg-white border-b-4 border-green-800 p-4 shadow-md flex justify-between items-center z-10 shrink-0">
         <div className="flex items-center gap-4">
           <div className="bg-green-800 text-white w-12 h-12 rounded-xl flex items-center justify-center font-black text-2xl">C</div>
         ...
         <div className={`flex flex-col items-center px-8 border-l-2 border-slate-200 ${timeLeft < 300 ? 'text-red-600 animate-pulse' : 'text-green-900'}`}>
       ...
       <div className="flex-1 flex flex-col lg:flex-row overflow-y-auto lg:overflow-hidden">
             ...
             <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-4 rounded-3xl border-2 border-slate-300 shadow-md gap-3">
                   ...
                   {currentIdx === questions.length - 1 ? (
                     ...
                     <button onClick={() => setCurrentIdx(prev => prev+1)} className="w-full sm:w-auto px-12 py-4 bg-green-800 text-white font-black rounded-2xl border-b-4 border-green-950 uppercase text-xs">Berikutnya</button>
                   )}
                </div>
             </div>
          </main>
          ...
          <aside className="w-full lg:w-80 bg-white border-l-4 border-slate-300 overflow-y-auto p-6 shrink-0 custom-scrollbar lg:h-full flex flex-col">
             <div className="flex-1">
                ...
                <div className="grid grid-cols-5 gap-2 mb-10">
                   {questions.map((item, i) => {
                      ...
                      return (
                        <button key={item.id} onClick={() => setCurrentIdx(i)} className={`h-12 w-full flex items-center justify-center rounded-xl font-black text-sm border-b-4 transition-all ${i === currentIdx ? 'scale-105 ring-4 ring-green-100 z-10' : ''} ${isDbt ? 'bg-orange-500 text-white border-orange-700' : hasAns ? 'bg-green-800 text-white border-green-900' : 'bg-white text-slate-400 border-slate-200'}`}>{i + 1}</button>
                      );
                   })}
                </div>
             </div>
             ...
             <div className="mt-auto border-t-2 border-slate-100 pt-6">
                ...
                <div className="bg-green-50 p-5 rounded-[2rem] border border-green-100 space-y-4">
                   <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-green-100 text-green-800 rounded-full flex items-center justify-center shrink-0">
       ...
       {isAutoSubmitting && (
         <div className="fixed inset-0 z-[1000] bg-green-950/80 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-500">
           <div className="bg-white rounded-[3rem] p-10 max-lg w-full shadow-2xl text-center space-y-6">
              <div className="relative w-24 h-24 mx-auto">
                ...
                <div className="absolute inset-0 border-8 border-green-800 rounded-full border-t-transparent animate-spin"></div>
              </div>
       ...
       {zoomImage && (
         <div className="fixed inset-0 z-[999] bg-green-950/90 backdrop-blur-md flex items-center justify-center p-4 md:p-12 animate-in fade-in duration-300" onClick={() => setZoomImage(null)}>
