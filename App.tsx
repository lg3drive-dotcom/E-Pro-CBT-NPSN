...
type ViewMode = 'login' | 'confirm-data' | 'quiz' | 'result' | 'admin-auth' | 'admin-panel' | 'analysis-panel' | 'ai-lab';
...
const App: React.FC = () => {
  ...
  const handleViolation = (reason: string) => {
    ...
    setView('login');
    setIdentity({ name: '', className: '', npsn: '', birthDate: '', token: '' });
  };
  ...
  const handleQuickDownloadRecap = async () => {
    if (!quickDownloadToken.trim()) return alert("Silakan masukkan token.");
    setIsQuickDownloading(true);
    try {
      const data = await fetchSubmissionsByToken(quickDownloadToken.toUpperCase());
      if (data && data.length > 0) {
        exportSubmissionsToExcel(data, `Rekap_Ringkas_${quickDownloadToken.toUpperCase()}`, questions);
  ...
  const handleImportQuestions = (imported: Question[], mode: 'replace' | 'append' = 'append') => {
    ...
    <div className="min-h-screen bg-white font-inter text-slate-800">
      ...
      {view === 'admin-panel' && (
        <div className="min-h-screen bg-white flex flex-col lg:flex-row relative">
          ...
          {showPusatLock && (
            <div className="fixed inset-0 z-[100] bg-green-950/80 backdrop-blur-xl flex items-center justify-center p-6">
              <div className="bg-white rounded-[3rem] p-10 max-md w-full shadow-2xl border border-slate-200 text-center animate-in zoom-in duration-300">
                ...
                <form onSubmit={verifyPusatCode} className="space-y-4">
                    ...
                    className="w-full p-4 bg-green-50 border-2 border-green-100 rounded-2xl text-center font-black outline-none focus:border-red-500 transition-all"
                    autoFocus
                  />
                  <div className="flex gap-2">
                    ...
                    <button type="submit" className="flex-[2] bg-green-900 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl">Verifikasi</button>
                  </div>
                </form>
              </div>
            </div>
          ...
          <header className="lg:hidden bg-green-950 text-white p-4 flex justify-between items-center sticky top-0 z-50">
            <div className="font-black text-xl flex items-center gap-2"><div className="w-6 h-6 bg-green-700 rounded"></div>E-PRO CBT SERVER</div>
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 bg-white/10 rounded-lg">Menu</button>
          </header>

          <aside className={`${isMobileMenuOpen ? 'flex' : 'hidden'} lg:flex w-full lg:w-72 bg-green-950 text-white flex-col p-6 lg:sticky top-0 lg:h-screen z-40 transition-all`}>
            <div className="hidden lg:flex font-black text-2xl mb-12 items-center gap-2"><div className="w-8 h-8 bg-green-700 rounded-lg"></div>E-PRO CBT SERVER</div>
            <nav className="space-y-2 flex-1">
              <button 
                onClick={() => {setAdminSubView('bank-soal'); setIsMobileMenuOpen(false);}}
                className={`w-full text-left p-4 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all ${adminSubView === 'bank-soal' ? 'bg-white/10 border-l-4 border-green-500 text-white' : 'text-slate-400 hover:bg-white/5'}`}
              >
                Bank Soal
              </button>
              <button 
                onClick={handlePusatAccess}
                className={`w-full text-left p-4 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all ${adminSubView === 'admin-pusat' ? 'bg-white/10 border-l-4 border-green-500 text-white' : 'text-slate-400 hover:bg-white/5'}`}
              >
                Admin Pusat
              </button>
              <button 
                onClick={() => {setView('ai-lab'); setIsMobileMenuOpen(false);}}
              ...
              <button onClick={() => setShowGuide(true)} className="w-full flex items-center gap-3 p-4 hover:bg-white/5 rounded-xl font-bold uppercase text-[10px] tracking-widest text-green-400 border border-transparent hover:border-green-500/20 transition-all">Panduan</button>
            </nav>
            ...
            <div className="mt-8 lg:mt-auto space-y-4">
              ...
              <div className="p-4 bg-white/5 border border-white/10 rounded-2xl space-y-3">
                 ...
                 <button 
                   ...
                   className={`w-full py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                     syncStatus === 'loading' ? 'bg-green-600 cursor-wait' : 
                     syncStatus === 'success' ? 'bg-green-500' : 
                     syncStatus === 'dirty' ? 'bg-green-700 shadow-lg' : 
                     'bg-white/10 hover:bg-white/20'
                   } text-white`}
                 >
                   {syncStatus === 'loading' ? 'MENGIRIM...' : syncStatus === 'success' ? '✓ BERHASIL TERKIRIM' : syncStatus === 'dirty' ? 'KIRIM KE CLOUD' : 'KIRIM KE CLOUD'}
                 </button>
              </div>
      ...
      {view === 'login' && (
        <div className="min-h-screen flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col md:flex-row border border-slate-200">
            <div className="md:w-5/12 bg-green-950 p-12 text-white flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-3 mb-12"><div className="w-10 h-10 bg-green-700 rounded-xl flex items-center justify-center font-black">C</div><div className="font-black text-2xl">E-Pro CBT</div></div>
                <h1 className="text-4xl font-black mb-6 leading-tight">Computer Based Test</h1>
                <p className="text-green-200/60 font-medium italic text-xs leading-relaxed">Pastikan guru mengunduh data Rekap Nilai setelah ujian selesai, karena sewaktu-waktu dapat hilang</p>
              </div>
              <div className="space-y-4">
                ...
                <button onClick={() => setView('analysis-panel')} className="w-full bg-green-600/10 hover:bg-green-600/20 p-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-green-500/30 transition-all text-green-400">📊 ANALISIS HASIL</button>
                <p className="text-[10px] text-center font-bold text-green-500/50 uppercase">{currentLinks.adminEmailDisplay}</p>
              </div>
            </div>
            <div className="md:w-7/12 p-8 lg:p-12 bg-white flex flex-col justify-center">
              ...
              <form onSubmit={handleStartQuiz} className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <input required type="text" placeholder="Nama Peserta" className="w-full p-4 bg-green-50 border-2 border-green-100 rounded-2xl outline-none focus:border-green-700 transition-all font-bold" value={identity.name} onChange={e => setIdentity({...identity, name: e.target.value})} />
                  <input required type="text" placeholder="Kelas" className="w-full p-4 bg-green-50 border-2 border-green-100 rounded-2xl outline-none focus:border-green-700 transition-all font-bold" value={identity.className} onChange={e => setIdentity({...identity, className: e.target.value})} />
                </div>
                <input required type="text" placeholder="NPSN" className="w-full p-4 bg-green-50 border-2 border-green-100 rounded-2xl outline-none focus:border-green-700 transition-all font-bold" value={identity.npsn} onChange={e => setIdentity({...identity, npsn: e.target.value.replace(/\D/g, '')})} />
                <div className="relative">
                  <span className="absolute left-4 -top-2.5 px-2 bg-white text-[10px] font-black text-green-700 uppercase tracking-widest z-10 border border-green-100 rounded-full">Tanggal Lahir</span>
                  <input required type="date" className="w-full p-4 bg-green-50 border-2 border-green-100 rounded-2xl outline-none focus:border-green-700 transition-all font-bold text-slate-700 min-h-[60px]" value={identity.birthDate} onChange={e => setIdentity({...identity, birthDate: e.target.value})} />
                </div>
                <input required type="text" placeholder="KODE TOKEN" className="w-full p-5 bg-green-50 border-2 border-green-200 rounded-2xl font-black text-green-800 text-center uppercase tracking-[0.3em] outline-none" value={identity.token} onChange={e => setIdentity({...identity, token: e.target.value})} />
                <button disabled={isSyncing} className="w-full font-black py-4 rounded-[2rem] text-lg bg-green-800 hover:bg-green-900 text-white shadow-xl transition-all">MASUK KE UJIAN</button>
              </form>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[4rem] p-12 max-w-5xl w-full shadow-2xl overflow-hidden relative">
             <div className="relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                   <div className="bg-green-50 p-8 rounded-[2.5rem] border-2 border-green-100 text-left space-y-6">
                      <h3 className="font-black text-slate-800 uppercase text-xs tracking-widest flex items-center gap-2">
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                        Rekap Nilai Cepat (.CSV)
                      </h3>
                      <div className="space-y-3">
                         <input type="text" placeholder="Input Token..." className="w-full p-3 bg-white border border-green-200 rounded-xl text-xs font-black uppercase tracking-widest outline-none focus:border-green-500" value={quickDownloadToken} onChange={e => setQuickDownloadToken(e.target.value)} />
                         <button onClick={handleQuickDownloadRecap} disabled={isQuickDownloading} className="w-full bg-green-600 hover:bg-green-700 text-white font-black py-4 rounded-xl text-[10px] uppercase tracking-widest transition-all">
                   ...
                   <div className="bg-green-50 p-8 rounded-[2.5rem] border-2 border-green-100 text-left space-y-6">
                      ...
                      <div className="space-y-3">
                         <input type="text" placeholder="Input Token..." className="w-full p-3 bg-white border border-green-200 rounded-xl text-xs font-black uppercase tracking-widest outline-none focus:border-green-700 mb-2" value={fullDownloadToken} onChange={e => setFullDownloadToken(e.target.value)} />
                         <input type="text" placeholder="NPSN (Opsional)" className="w-full p-3 bg-white border border-green-200 rounded-xl text-xs font-bold outline-none focus:border-green-700" value={fullDownloadSchool} onChange={e => setFullDownloadSchool(e.target.value.replace(/\D/g, ''))} />
                         ...
                         <button onClick={handleFullDataDownload} disabled={isFullDownloading} className="w-full bg-green-800 hover:bg-green-900 text-white font-black py-4 rounded-xl text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2">
                ...
                <div className="mb-10">
                   <a 
                     ...
                     className="w-full inline-flex items-center justify-center gap-3 bg-gradient-to-r from-green-600 to-green-800 hover:from-green-700 hover:to-green-900 text-white font-black py-5 px-10 rounded-[2rem] shadow-xl shadow-green-900/20 transition-all active:scale-95 text-xs uppercase tracking-[0.2em]"
                   >
      ...
      {view === 'ai-lab' && <AiQuestionLab onBack={() => setView('admin-panel')} onImport={handleImportQuestions} />}
      ...
      {view === 'result' && lastResult && (
        <div className="min-h-screen flex flex-col items-center justify-center p-4">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl max-w-md w-full border border-slate-200 text-center">
             ...
             <div className="bg-green-800 p-8 rounded-[2.5rem] mb-10 shadow-xl">
                <p className="text-[10px] font-black text-green-200 uppercase tracking-widest mb-2">Skor Anda</p>
                <p className="text-7xl font-black text-white">{lastResult.score.toFixed(1)}</p>
             </div>
             <div className="space-y-4">
                <button onClick={() => generateResultPDF(lastResult, questions.filter(q => !q.isDeleted))} className="w-full bg-green-50 text-green-800 font-black py-4 rounded-2xl border-2 border-green-200 uppercase text-[10px] tracking-widest">DOWNLOAD HASIL PDF</button>
                <button onClick={() => window.location.reload()} className="w-full bg-green-950 text-white font-black py-4 rounded-2xl transition-all uppercase tracking-widest text-[10px]">KEMBALI KE LOGIN</button>
             </div>
          </div>
        </div>
      )}
